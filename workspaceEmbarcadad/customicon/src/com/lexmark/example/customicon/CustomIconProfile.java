...
public class CustomIconProfile implements PrtappProfile, WelcomeScreenable,
	...
	private void MenuTipoBusca(BasicProfileContext context)
		...
		promptMenuPrincipal.setLabelFalse("Matrícula");
	...
	public boolean promptIndexacaoData(PromptFactory pf,
			BasicProfileContext context) {
		boolean retorno = true;

		MessagePrompt messageConfirmStudent = null;
		try {
			messageConfirmStudent = (MessagePrompt) pf
					.newPrompt(MessagePrompt.ID);
		} catch (PromptFactoryException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		messageConfirmStudent.setLabel("Atenção");
		messageConfirmStudent.setMessage("Digite uma data Valida");
		...
		StringPrompt promptDataInputName = null;
		try {
			promptDataInputName = (StringPrompt) pf.newPrompt(StringPrompt.ID);
		} catch (PromptFactoryException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		promptDataInputName
				.setLabel("Informe a data:    (Formato DD/MM/AAAA)   ");

		String dataAtributo = "";
		String identificacaoAtributo = "";

		String data = "#D";
		String identificacao = "#I";

		try {
			context.displayPrompt(promptDataInputName, TIMEOUT);
		...
		try {
			...
			String dateInString = promptDataInputName.getValue();

			Date date = formatter.parse(dateInString);
			dataAtributo = dateInString;
			retorno = true;
		} catch (Exception e) {
			retorno = false;
	...
	private boolean ProcessScans(BasicProfileContext context, String fileName,
			String Document, String ra, String categoria, String Identificacao,
			String Data) {
		...
		if (sdf != null) {
			Activator
					.getLog()
					.debug("--------Disk dif CARALHOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO  --------------");
		} else {
			Activator
					.getLog()
					.debug("--------Disk  é null   CARALHOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO-------------");
		}
		try {
			return Scan.exec(sdf, context, fileName, Document, null, ra,
					categoria,Data,Identificacao);
	...
	private boolean ProcessScans(BasicProfileContext context, String fileName,
			String Document, String ra, String categoria) {
	...
	private void FinnalyScanDocument(BasicProfileContext context,
			Student student, Document document, String identificacao,
			String data, boolean envioIdentificacao) throws PromptException {
		Activator.getLog().debug(
		...
		if (document != null) {
			...
			Activator
					.getLog()
					.debug("--------Docuemnto Diferente de nuloooooooooo  --------------");

			String fileNameOut = student.getRA() + "_" + getDateNow();

			String FileNameImge = fileNameOut + ".tif";

			if (!envioIdentificacao) {

				if (ProcessScans(context, FileNameImge, document.getName(),
	...
	private void getDocument(BasicProfileContext context, Student student,
		...
		for (int k = 0; k < document.length; k++) {
			listNamedoc[k] = document[k].getName()+" : "+document[k].getCod() ;
		}
		...
		try {
			...
			if (comBoListDoc.getDismissButton().equalsIgnoreCase("back")) {
				...
				promptInputName.setLabel("Informe a Identificação  do aluno:");

				StringPrompt promptDataInputName = (StringPrompt) pf
						.newPrompt(StringPrompt.ID);
				promptDataInputName
						.setLabel("Informe a data  do aluno:    (Formato DD/MM/AAAA)   ");
				...
				String entrada = document[comBoListDoc.getSelection()]
						.getName();
				if (entrada.indexOf(data) != -1) {
					context.displayPrompt(promptDataInputName, TIMEOUT);
					try {
						verifica = true;
						...
						try {
							...
							while (!promptIndexacaoData(pf, context)) {
								promptIndexacaoData(pf, context);
							}
						}
				...
				if (entrada.indexOf(identificacao) != -1) {
					verifica = true;
					context.displayPrompt(promptInputName, TIMEOUT);
					identificacaoAtributo = promptInputName.getValue();
				}
				if (entrada.indexOf(data) != -1
						|| entrada.indexOf(identificacao) != -1) {
					MessagePrompt finalPrompt = (MessagePrompt) pf
							.newPrompt(MessagePrompt.ID);
					finalPrompt.setLabel("Atenção");
					finalPrompt.setMessage("Data: " + dataAtributo
							+ " Identificação: " + identificacaoAtributo);

					context.displayPrompt(finalPrompt, TIMEOUT);

				}

				Activator.getLog().debug("--------FINALIZANDO --------------");
				FinnalyScanDocument(context, student,
						document[comBoListDoc.getSelection()],
						identificacaoAtributo, dataAtributo,verifica);
			}
		} catch (PromptFactoryException e) {
			Activator.getLog().debug(
	...
	private void findStudentByRA(BasicProfileContext context)
		...
		promptInterRa.setLabel("Informe o número da matrícula do aluno:    (Versão:1.0 )");
