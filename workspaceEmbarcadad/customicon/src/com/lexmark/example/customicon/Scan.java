...
public class Scan {
	...
	public static boolean exec(StorageDevice DISK, BasicProfileContext context,
			String fileNameOut, String docType, Document doc, String ra,
			String categoria, String data, String Identificacao)
			throws SocketException, IllegalStateException, IOException,
			Exception {
		...
		color.setInfo(new IntegerElem(0));
		color.lock();
		...
		WorkflowSetting monoBity = dw.getSettingCollection().getSetting(
				"monoBitdepth");
		monoBity.setInfo(new IntegerElem(1));
		monoBity.lock();

		File file = new File(DISK.getRootPath().toString() + "/" + getDateNow()
				+ ".tif");
		...
		if (cr) {
			...
			Activator
					.getLog()
					.debug("----------------------------- CRIAAAAAAAAAAAAAAAAAAAAAAADO MAAAAAAAAAAAAAAAAAAAAAAAANO: -----------------------------  ");
		}
		ScanDocuments scanDoc = new ScanDocuments(file);
		...
		try {
			...
			new SendFile(file, ra, categoria, data, Identificacao);
			return true;
	...
	public static boolean exec(StorageDevice DISK, BasicProfileContext context,
			String fileNameOut, String docType, Document doc, String ra,
			String categoria) throws SocketException, IllegalStateException,
		...
		originalMediaSize.setInfo(new IntegerElem(4));
		originalMediaSize.lock();
		if (categoria.toUpperCase().indexOf("#PB") > -1) {

			resolution = dw.getSettingCollection().getSetting("resolution");
			resolution.setInfo(new IntegerElem(150));
			resolution.lock();
			...
			contentType = dw.getSettingCollection().getSetting("contentType");
			contentType.setInfo(new IntegerElem(0));
			contentType.lock();

			originalMediaSize = dw.getSettingCollection().getSetting(
					"originalMediaSize");
			originalMediaSize.setInfo(new IntegerElem(4));
			originalMediaSize.lock();
		...
		/*
		 * WorkflowSetting color = dw.getSettingCollection().getSetting(
		 * "color"); color.setInfo(new IntegerElem(1)); color.lock();
		 */
		...
		try {
			...
			new SendFile(file, ra, categoria);
			return true;
